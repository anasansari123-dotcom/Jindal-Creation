    ...
    <header className="sticky top-0 z-20 flex h-14 sm:h-16 items-center justify-between gap-2 border-b border-gray-100 bg-white px-3 sm:px-4 lg:px-6">
      <div className="relative ml-10 sm:ml-12 lg:ml-0 flex-1 min-w-0 max-w-md">
        ...
        {showResults && searchResults.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 rounded-lg border bg-white shadow-lg z-50">
            {searchResults.map((r, i) => (
              ...
              >
                ...
                <span className="ml-2 text-gray-400 text-xs">{r.sublabel}</span>
                <span className="ml-2 text-xs text-gold capitalize">{r.type}</span>
              </Link>
            ))}
          </div>
        )}
